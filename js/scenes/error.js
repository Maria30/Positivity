...
class Error extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(config.width*.5, config.height*.65, "Une erreur s'est produite", {fontFamily: 'Normal', align: 'center', fontSize: '4em', color:'#444', wordWrap: {width: config.width*.8}}).setOrigin(.5, .5)
    
    this.add.text(config.width*.5, config.height*.75, "Nous sommes désolés, il semblerait que Positivity a eu un problème...", {fontFamily: 'Normal', align: 'center', fontSize: '2em', color:'#444', wordWrap: {width: config.width*.8}}).setOrigin(.5, .5)
