class ThanksJPO extends Phaser.Scene {
  constructor() {
    super("thanksJPO")
  }
  ...
  create() {
    ...
    this.add.text(config.width*.5, config.height*.1, "Merci beaucoup d'avoir joué à la première version de", {fontFamily: 'Normal', fontSize: '4em', color:'#444'}).setOrigin(0.5, 0.5)
    this.title = this.add.image(config.width*.5, config.height*.3, "title").setOrigin(0.5, 0.5)
    this.title.setScale(.5)
    this.add.text(config.width*.5, config.height*.45, "La version complète de notre jeu sera disponible à la fin du mois de mars.", {fontFamily: 'Normal', fontSize: '2.5em', color:'#444'}).setOrigin(0.5, 0.5)
    this.add.text(config.width*.5, config.height*.50, " En attendant, n'hésitez pas à vous abonner à notre page Instagram et à remplir notre formulaire de satisfaction !", {fontFamily: 'Normal', fontSize: '2.5em', color:'#444'}).setOrigin(0.5, 0.5)
    ...
    this.clickPlay = this.add.text(config.width*.5, config.height*.9, "Rejouer", {fontFamily: 'Normal', fontSize: '4em', color:'#fff'}).setInteractive({cursor: 'pointer'}).setOrigin(0.5, 0.5)
